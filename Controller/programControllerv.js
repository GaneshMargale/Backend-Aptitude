...
// const CodeSnippet = require('../Models/codeSnippetsModel');
// const TestCases = require('../Models/testModel');
// const javaPath = `${__dirname}/../lib/Java/jdk-21/bin`;
...
exports.runProgram = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // Compile Java files
    compileJavaFiles(mainFilePath, solutionFilePath, (compileError) => {
      if (compileError) {
        // Handle compilation error
        return next(new AppError('Compilation error', 500));
      }

      // Continue with the rest of your logic, such as running the Java program
      createInputFile(inputFilePath, test.testCases);
      compileAndRunJava(
        mainFilePath,
        solutionFilePath,
...
function compileJavaFiles(mainFilePath, solutionFilePath, callback) {
  // Compile Main.java and Solution.java
  exec(`javac ${mainFilePath} ${solutionFilePath}`, (error, stdout, stderr) => {
    if (error) {
